...
GOALDIR_KEY = (
    ...
    or os.getenv("GOAL_API_KEY")
    or os.getenv("GOALDIR_KEY")
    or ""
).strip()

GOALDIR_BASE = os.getenv(
    "GOALDIR_BASE_URL",
...
ENABLE_LIVE_EXECUTION = (
    os.getenv("ENABLE_LIVE_EXECUTION", "false").lower() == "true"
)
...
LINEUP_HORIZON_HOURS = float(
    os.getenv("LINEUP_HORIZON_HOURS", "8")
)

LINEUP_MAX_PER_RUN = int(
    os.getenv("LINEUP_MAX_PER_RUN", "18")
)


def load_leagues() -> dict:
    return json.loads(
        (ROOT / "config" / "leagues.json").read_text()
    )

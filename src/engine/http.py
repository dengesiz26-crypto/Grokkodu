...
def cached_get(
    ...
    *,
    ...
    # Goaldir must always use fresh API data.
    ...
    # No application-side request limit.
    # Goaldir reaches the API on every call.
    r = SESSION.get(
        url,
        headers=headers or {},
        params=params or {},
    ...
    # Keep the response in the cache database for storage/history,
    # but Goaldir will never read it back as a cached response.
    c.execute(
        """
...
def budget_today(provider="goaldir") -> int:
    ...
    There is NO request limit here.
    """
    init()
